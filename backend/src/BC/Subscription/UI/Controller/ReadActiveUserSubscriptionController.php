...
class ReadActiveUserSubscriptionController extends Controller {
    ...
    public function __construct(private readonly ReadActiveUserSubscriptionUseCase $useCase) {}

    public function __invoke(string $id): JsonResponse {
        $subscription = $this->useCase->execute($id);

        if (!$subscription) {
            return response()->json(['status' => 'error', 'error' => 'Suscripcion activa no encontrada'], 404);
        }
